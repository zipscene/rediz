...
class RedisClientCommon extends EventEmitter {
	...
	_registerAllScriptsToClient() {
		return pasync.eachSeries(_.keys(this.registeredScripts), (scriptName) => {
			return this._registerScriptToClient(scriptName);
		});
	}
	...
	runScript(name, ...args) {
		...
		if (script.sha) {
			return this.evalsha(script.sha, script.options.numKeyArgs || 0, ...args);
		} else {
			return this.eval(script.scriptText, script.options.numKeyArgs || 0, ...args);
		}
	}
